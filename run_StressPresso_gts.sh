...
# data config
repo_id="slprl/StressPresso"
data_root="data/StressPresso"
download_dir=${data_root}/raw
extract_root=${data_root}/audio
audio_dir=${data_root}/audio
json_root=data-json/StressPresso
prompt_file=/datas/store162/annhung/Qwen3-SLU/prompt/prompt_gts.txt   # 可指定外部 prompt 檔案，空字串則使用 prepare_macslu_jsonl.py 內建 prompt
target="text_gts"
...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
    echo "Stage 0: Download StressPresso and prepare jsonl"

    prep_cmd=(
        python local/prepare_StressPresso_jsonl.py
        --repo-id "$repo_id"
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
    echo "Stage 2: Inference on StressPresso test"
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
    echo "Stage 3: Evaluate StressPresso predictions"
    ...
    for test_set in $test_sets; do
        ...
        python local/metrics_TinyStress.py "$pred_file" "$gt_file" "$target" "StressPresso" | tee ${exp_root}/${test_set_name}/${test_set}/metrics.txt
    done
fi

if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
    echo "Stage 4: Summary (StressPresso)"
