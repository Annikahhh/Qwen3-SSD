    ...
    for split in args.splits:
        ...
        with out_path.open("w", encoding="utf-8") as f:
            for i, ex in enumerate(tqdm(ds[split])):
                # 1. 處理並保存音檔
                wav_filename = f"StressTest_{split}_{i}.wav"
                ...
                # 2. 根據索引提取重音詞
                indices = []
                if "emphasis_indices" in ex:
                    indices = ex.get("emphasis_indices", [])
                elif "indices" in ex.get("stress_pattern", {}):
                    indices = ex.get("stress_pattern", {}).get("indices", [])
                ...
                # gender
                gender = ex.get("metadata", {}).get("gender", "nan")
                tasks = {
                    "gender": gender,
                    "stress_pattern": stress_pattern
                }
                ...
                row = {
                    "text_id": f"StressTest_{split}_{i}",
