...
# data config
repo_id="slprl/StressTest"
data_root="data/StressTest"
download_dir=${data_root}/raw
extract_root=${data_root}/audio
audio_dir=${data_root}/audio
json_root=data-json/StressTest
...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
    echo "Stage 0: Download StressTest and prepare jsonl"

    prep_cmd=(
        python local/prepare_StressTest_jsonl.py
        --repo-id "$repo_id"
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
    echo "Stage 2: Inference on StressTest test"
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
    echo "Stage 3: Evaluate StressTest predictions"
    ...
    for test_set in $test_sets; do
        ...
        python local/metrics_TinyStress.py "$pred_file" "$gt_file" "$target" "StressTest" | tee ${exp_root}/${test_set_name}/${test_set}/metrics.txt
    done
fi

if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
    echo "Stage 4: Summary (StressTest)"
